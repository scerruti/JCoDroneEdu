...
public class AdvancedPatternDrone extends BasicPatternDrone {
    ...
    public void circle(int speed, int direction, double duration) {
        ...
        drone.sendControl(0, speed, direction * speed, 0);
    ...
    /**
     * Flies the drone in a spiral pattern, expanding outward.
     * 
     * This pattern teaches:
     * - Variable distances in loops
     * - Mathematical progression
     * - Gradual change over time
     * 
     * @param startRadius Starting radius in centimeters
     * @param endRadius Ending radius in centimeters
     * @param turns Number of complete turns to make
     * @param speed Flight speed as percentage
     */
    public void spiral(int startRadius, int endRadius, int turns, int speed) {
        if (startRadius < 10 || startRadius > 50) {
            throw new IllegalArgumentException("Start radius must be between 10 and 50 cm");
        }
        if (endRadius < startRadius || endRadius > 150) {
            throw new IllegalArgumentException("End radius must be larger than start radius and max 150 cm");
        }
        if (turns < 2 || turns > 8) {
            throw new IllegalArgumentException("Turns must be between 2 and 8");
        }
        if (speed < 10 || speed > 100) {
            throw new IllegalArgumentException("Speed must be between 10 and 100 percent");
        }
        
        int segmentsPerTurn = 8; // 8 segments per full rotation
        int totalSegments = turns * segmentsPerTurn;
        int turnAngle = 360 / segmentsPerTurn;
        
        // Calculate how much to increase radius each segment
        ...
        for (int segment = 0; segment < totalSegments; segment++) {
            ...
            // Calculate segment length for current radius
            double circumference = 2 * Math.PI * currentRadius;
            int segmentLength = (int) Math.round(circumference / segmentsPerTurn);
            
            drone.go("forward", segmentLength, speed);
            drone.go("cw", turnAngle, 25);
            drone.hover(0.1);
        }
    }
